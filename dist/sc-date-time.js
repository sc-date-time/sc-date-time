      ...
      controller: [
        '$scope', 'scDateTimeI18n', function(scope, scDateTimeI18n) {
          ...
          scope.$watch('clock._minutes', function(val, oldVal) {
            if ((val != null) && !isNaN(val) && (0 <= val && val <= 59)) {
              scope.clock._minutesString = val > 9 ? val.toString() : ("0" + val).slice(-2);
              if (val !== scope.date.getMinutes()) {
